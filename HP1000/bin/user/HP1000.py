...
All packets have the same basic structure: the 8-byte (null terminated) sending
...
system for the LOOP packet and so will convert all values to SI units.

To find out what units are being sent by the weather station, send a "SETUP" command and
...
Many of these (from offset 0x37 on) are of little interest for the driver but the
...
The HISTORY_FILE command will return a packet structured (after the header) as:
...
data. The RecCount values correspond to the years (e.g. the first one is for the latest
year, then 2nd one for the previous year etc.)

Note: There may be a 'bug' in the HP1000 firmware in that the current year can be shown as
having 0 records. Whether this is a bug or not, this is now treated as a special case and an
estimate of the number of records is made based on the 'day of the year' the catchup is occurring.
...
The driver then starts reading HISTORY_DATA records, 16 at a time if possible) from
...
class HP1000Driver(weewx.drivers.AbstractDevice):
            ...
            if not self.internal_test_mode:
                ...
                while True:
                    ...
                    try:
                        ...
                        self.ws_socket.setblocking(0)
                        self.ws_socket.settimeout(self.socket_timeout)
                        break
                    except socket.timeout:
                        retry_counter -= 1
                        if retry_counter == 0:
        ...
        while True:
            ...
            if self.internal_test_mode:
                ...
                if self.iteration_count > self.max_iterations:
                    loginf("glp - exit with iteration_count = {0}".format(self.iteration_count))
                    sys.exit()
    ...
    def recvWithTimeout( self, sock, size):
    ...
    def getHistoryData(self, year, record_count, starting_record):
        ...
        while rx_data is None:
            try:
                rx_data = self.recvWithTimeout(self.ws_socket, record_count * 60 + 40)
            except Exception as e:
        ...
        while len(rx_data) < pkt_length:
            ...
            extra_data = self.recvWithTimeout( self.ws_socket, 1024)
    ...
    def genStartupRecords(self, lastTimestamp):
        ...
        while year_index >= 0:
            ...
            year_count = (struct.unpack('h', rxData[32:34])[0] - 40)//6 #packet size less the header
            year_data = []
            for i in range(year_count):
                year_number = struct.unpack('H', rxData[40+i*2:42+i*2])[0]
                record_count = struct.unpack('I', rxData[40+year_count*2 + i * 4:44 + year_count * 2 + i * 4])[0]
                if 0 == record_count:
                    # Calculate how many entries there could be this year
                    day_count = datetime.datetime.now().timetuple().tm_yday
                    record_count = day_count * 288;
                year_data.append([year_number, record_count])
            ...
            if lastTimestamp is None:
                ...
                year_index = [x for x, y in enumerate(year_data) if y[0] != 0][-1]
                year = year_data[year_index][0]
                start_record = 0
            else:
                start_date = datetime.datetime.fromtimestamp(lastTimestamp)
                last_record_date = start_date
                for index, year_info in enumerate(year_data):
                    ws_year = year_info[0]
                ...
                while True:
                    ...
                    if len(rec_data) < 80:
                        # We have asked for a record that does not exist
                        upper = sample - 1
                        continue
...
# TODO: This needs to be checked
            if year_index == 0 and start_record == year_data[year_index][1] - 1:
                print("Exception condition found")
#                break
            ...
            while year_index >= 0:
                year = year_data[ year_index][0]
                year_record_count = year_data[year_index][1]
                record_count = 16
                ...
                if record_count > 0:
                    ...
                    while record_count > 0:
                        ...
                        if last_record_date is not None and record_datetime <= last_record_date:
                            record_count = 0
                            year_index = -1
                            record_datetime = last_record_date
                            break
...
# Comment this out if we are doing local testing.
# Otherwise the function just exits immediately it is called
                        yield _packet
            ...
            print(f"Going aroung again - last timestamp: {record_datetime}")
...
#    station.internal_testing(True)
    station.genStartupRecords(time.mktime(datetime.datetime(2024, 1, 17).timetuple()))
